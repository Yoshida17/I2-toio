...
このページで紹介したキーボード操作のサンプルプログラムは，以下のファイルに用意されている．

### パターン1: 直接操作（ラジコン方式）

**ファイル**: `example/22_keyboard_radio.py`

実際に実行可能な完全なサンプルプログラム．矢印キーでキューブを操作し，qキーで終了．

```bash
# コマンドラインで実行
cd ~/2026_Engineering_Basic_Training_toio
python example/22_keyboard_radio.py
```

### パターン2: 角度制御による滑らかな移動

**ファイル**: `example/23_keyboard_grid.py`

マットを使った角度制御版のサンプルプログラム．矢印キーで4方向に移動し，qキーで終了．

```bash
# コマンドラインで実行
cd ~/2026_Engineering_Basic_Training_toio
python example/23_keyboard_grid.py
```
...
1. **Thonnyの設定**
   - Thonnyの出力ウィンドウをアクティブ（クリック）にしてから実行する
   - キーボード入力がThonnyに送られるよう確認

2. **suppress=True の設定**
   - `suppress=True`は他のアプリケーションにキーイベントを送らない設定
   - Thonnyがキーに応答しない場合，これが原因かもしれません
...
1. **複数キーの同時押しで予期しない動作**
   - キーボードハードウェアの制限で，3つ以上のキーを同時押しできないことがある
   - 2つのキーの同時押しなら大丈夫

2. **キーリピートによる問題**
   - キーを長押しするとリピート動作する
   - `current_keys`セットを使うことで，リピートを無視できる

### マットがない場合の動作

**パターン1（直接操作）**:
- マットなしでも動作します
- 進む・後退・回転のみで位置情報は使いません

**パターン2（角度制御）**:
- マットが必須です
- マットがないと，`get_orientation()`が`None`を返すため，動作しません
- エラーが出た場合，マットの上でプログラムを実行してください
...
```python
base_speed = 20  # この値を増やすと速くなる（50〜100も試す）
```

**キューブが振動的に動く**:

```python
turn_gain = 0.25  # この値を減らすと（0.1など）滑らかになる
```
...
キーボード操作をマスターしたら，以下のプログラムを作成してみよう：

1. **位置情報 + キーボード操作**: `day2/mat.md`と`day2/mat-control.md`と組み合わせて，キーボードで指定座標に移動するプログラム

2. **センサー入力 + キーボード操作**: 1日目で学んだセンサー（ボタン，傾き）とキーボードを組み合わせたプログラム

3. **コンテスト課題**: `day2/challenge-guide.md`で紹介している「宝探しゲーム」に挑戦
